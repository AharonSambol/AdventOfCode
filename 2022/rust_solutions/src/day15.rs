...
pub fn solve() {
    ...
    for line in data.split('\n') {
        let (s_x, s_y, b_x, b_y) = Regex::new(r"-?\d+").unwrap()
            .find_iter(line).map(
                |x| x.as_str().parse::<i32>().unwrap()
    ...
    loop {
        ...
        rxs.iter().filter_map(|x| x.try_recv().ok()).for_each(|x| {
            println!("part2: {x}");
            exit(0)
        })
    }
...
fn part1(sensors: &Vec<(i32, i32, i32)>, beacons: HashSet<(i32, i32)>, y: i32) -> i32 {
    let ranges: Vec<(i32, i32)> = sensors.iter().filter_map(
        |(s_x, s_y, r)| {
            let w = *r - (*s_y - y).abs();
            if w > 0 { Some((*s_x - w, *s_x + w)) } else { None }
        }
    ).collect();
    let mut res = 0;
    ...
    for (i, (mut r1_start, mut r1_end)) in ranges.iter().enumerate() {
        ...
        'lp: loop {
            for (j, (r2_start, r2_end)) in ranges.iter().enumerate().skip(i + 1) {
                ...
                if r1_start <= *r2_start && *r2_start <= r1_end || *r2_start <= r1_end && r1_end <= *r2_end ||
                    r1_start <= *r2_end && *r2_end <= r1_end || *r2_start <= r1_start && r1_start <= *r2_end {
...
fn part2(sensors: &Arc<Vec<(i32, i32, i32)>>, y: i32, mut start: i32, end: i32) -> Option<i64> {
    let ranges = sensors.iter().filter_map(
        |(s_x, s_y, r)| {
            let w = r - (s_y - y).abs();
            if w > 0 && s_x - w <= end {
                Some((s_x - w, s_x + w))
