...
fn val(grid: &Vec<Vec<i32>>, r: usize, c: usize, part1: bool) -> i32 {
    ...
    if part1 {
        (dirs
            .iter()
            .map(|x| *x.iter().max().unwrap_or(&-1))
            .min()
            .unwrap()
            < grid[r][c]) as i32
    } else {
        dirs.iter()
            .map(|dr| len_until_limit(dr, grid[r][c]))
            .reduce(|x, y| x * y)
            .unwrap() as i32
    }
}
...
fn len_until_limit(lst: &Vec<i32>, limit: i32) -> usize {
    lst.iter()
        .enumerate()
        .filter_map(|(i, x)| if *x >= limit { Some(i + 1) } else { None })
        .next()
        .unwrap_or(lst.len())
}
